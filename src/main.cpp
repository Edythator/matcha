...
std::string get_string(int pos, const std::vector<unsigned char>& buffer)
{
	...
	return std::string(buffer.begin() + pos + 1, buffer.begin() + pos + length);
}
...
int main(int argc, char* argv[])
{
	...
	for (size_t i = 2; i < buffer.size(); i++)
	{
		switch (buffer[i])
		{
		case INT: variables.push_back(Variable{ INT, buffer[i + 1], buffer[i + 2] });
			i += 2;
			break;
		case BOOL: variables.push_back(Variable{ BOOL, buffer[i + 1], buffer[i + 2] });
			i += 2;
			break;
		case STR: strings.push_back(String{ STR, buffer[i + 1], get_string(i + 2, buffer) }); break;
